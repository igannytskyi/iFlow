...
def emitted():
    return set(re.findall(r'self\.fail\("([^"]+)"', GATE.read_text()))


def cr_003_01():
    claimed = {rid for rid, mark, _ in rules() if mark == "M"}
    missing = sorted(claimed - emitted())
    return missing and f"claimed mechanical, never emitted: {', '.join(missing)}" or None
...
def cr_003_04():
    ...
    for name in ("clock-instant", "placeholder-filter"):
        got += [l.replace(name, f"changes/{name}", 1)
                for l in _gate_on(ROOT / "changes" / name).splitlines() if l]
