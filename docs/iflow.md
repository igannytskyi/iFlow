...
- **P7** `Trace`, with the identity and version of the executor at the time of the run, **and whether that run is repeatable** — a deterministic transformation on fixed inputs can be re-run to the same result and its trace is derived; an agent's cannot, and its trace is testimony about something that happened once.
