...
| **R12** | An arbiter is accepted only once shown to fail for the stated reason | `M` | A unit that is preparatory and whose scope is an arbiter path must appear in the execution stage's arbiter-acceptance table. A change with no such table predates the obligation and is noted |
