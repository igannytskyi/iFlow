...
| **R16** | A criterion that needs an evidence plan has one, derived from the estate | `M` | The specification says per criterion whether a plan is needed; a needed plan must exist in admission and must name what is observed, by what method, and the area of effect it was derived from. What the plan names is then put to the estate: ground the change provably does not reach is not an area of effect, and a plan that names it was derived from something else. Where the estate cannot answer, the gate says so rather than passing in silence. The estate is asked the other way too: ground the change reaches firmly, which no test names and no plan observes, is an area left judged by nothing and is refused — not every area a change reaches, which on a central module is hundreds, but the part nothing else watches. A specification that does not declare this predates the obligation and is noted |
| **R21** | Work that landed having touched ground nothing reaches from its scope owes that edge to the model | `M` | The area's own parameter says being wrong is not detectable from the source and is corrected only from outside; a landing is that outside. Where a unit's candidate holds a file the estate does not reach from that unit's scope, the edge exists and no re-reading finds it — reflection, a framework loading by name, a path built at run time. The gate refuses until it is recorded, and what is recorded is answered with as observed rather than derived, outranking what reading found. Kept where the cache is not: rebuilding the index loses nothing a landing taught |
| **R22** | Nothing enters on a run nobody made | `M` | Two escalations said this from different ends: a claim that a run repeats was checked for a value being present and never by running it, and entry happened before the whole set of arbiters had been run, so an invalidated verdict was found after the fact. Both are the same gap — the record of a run trusted where the run itself was available. A landing now requires that the arbiters have been run against exactly this framework and that every claim to repeat has been repeated against exactly this command. What is kept is not a record to be believed but a note that the run has just been made against this content, derived and disposable, and any edit to that content voids it. A change checked away from the framework that judges it cannot be asked the question and is noted |
...
| **R13** | A claim that a run repeats says what to re-run, and survives being re-run | `M` | Every evidence row carries a defined `Repeatable` value, and a `yes` names its command in backticks — a claim naming nothing cannot be tested and is therefore not a claim, and a sentence describing a run is not a command. `check.py --repeat <slug>` runs exactly what is written there, arguments and all, and compares two runs. Reading only the path ran a tool without the argument that decides everything it does: twice, identically, and the claim passed on a comparison of two help screens. A change whose evidence has no such column predates the obligation and is noted |
...
- **What the estate model resolves.** Every answer about an estate rests on an index that resolves a name, not a type: a region can be named by a test that never exercises it, and reach can be missed where a framework loads what no import reaches. Resolving types belongs to the grammars and the indexes built on them; this layer is the one above, and what it loses by not doing that work is carried on every statement as its confidence rather than hidden. A rule resting on the model is as good as the model, and says so.
- **What it costs to trust nothing.** An edit anywhere inside the framework voids every note that its arbiters were run and every note that a claim to repeat was repeated, because a note about a run over other content says nothing about this one. In a repository whose changes are to the framework itself that means running them again after each change; for an estate whose changes are not, the notes stand until the framework moves.
- **Whether a debt was actually settled.** A closure is a claim by whoever wrote the row, in the same way a human act is under R15 and R18. Two of the three rules that once shared this limit escaped it — one by removing the record and asking the arbiters instead, one by re-running what a row names — and this one cannot: what a debt owed is stated in a sentence, and no artefact decides whether the sentence was answered. The ledger reports the debts a change closes and who closed them, and the reading of whether it was really closed is a human act.
