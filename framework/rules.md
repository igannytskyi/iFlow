...
| **R13** | A claim that a run repeats says what to re-run, and survives being re-run | `M` | Every evidence row carries a defined `Repeatable` value, and a `yes` must name something inside the repository that can be run again — a claim naming nothing cannot be tested and is therefore not a claim. `check.py --repeat <slug>` runs each and compares, so the claim is settled by re-running rather than believed. A change whose evidence has no such column predates the obligation and is noted |
