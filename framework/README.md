...
| | |
|---|---|
| `glossary.md` | What every word means, in a sentence or two. For a person |
| `conventions.md` | Every code, defined once. For the gate — nothing anywhere may use a value this file does not define |
...
| `estate.py` | What the estate is, derived from it rather than described about it. Every answer carries where it came from and how far to trust it, including what crosses between repositories |
...
`--unused` lists every column the gate does not read and every code nothing validates. It does not call them ceremony — most exist for a person — but before it nobody could list them, and three fields that had never once been filled in earnest were removed the first time it ran.
